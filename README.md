...
5. ใน Z80sim กด **L** → พิมพ์ชื่อไฟล์ hex (เช่น `lab1.h`) → Enter → โค้ดโหลดเข้า memory ตาม ORG

> ชื่อไฟล์ hex = ชื่อไฟล์เดิม (ตัดเป็น DOS 8.3 ไม่เกิน 8 ตัว) + `.h` เช่น `lab1.asm` → `lab1.h` — DOS เก็บบนดิสก์เป็น `LAB1.H` เสมอ แต่พิมพ์ตัวเล็กหรือใหญ่ก็โหลดได้
